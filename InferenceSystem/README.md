...
Linux:
```
docker run --rm -it --env-file .env -v $PWD/config:/usr/src/app/config live-inference-system python3 -u ./src/LiveInferenceOrchestrator.py --config ./config/Test/FastAI_LiveHLS_OrcasoundLab.yml
```

Windows:
```
docker run --rm -it --env-file .env -v %cd%/config:/usr/src/app/config live-inference-system python3 -u ./src/LiveInferenceOrchestrator.py --config ./config/Test/FastAI_LiveHLS_OrcasoundLab.yml
```
